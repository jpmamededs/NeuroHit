...
void game() {
  ...
  while (gameRunning) {
    ...
    if (elapsedTime >= 10000) {
      ...
      numLedsToTurnOn = min(numLedsToTurnOn + 1, 9); // Increment number of LEDs to turn on, up to 9
      ledOnTime = max(ledOnTime - 100, 300); // Decrease LED on time, down to 300ms
      Serial.print("New number of LEDs to turn on: ");
      Serial.println(numLedsToTurnOn);
      Serial.print("New LED on time: ");
    ...
    while (true) {
      if (millis() - lastButtonPressTime > ledOnTime) {
        Serial.println("Eroou!");
        gameRunning = false;  // End game if no button pressed within ledOnTime
